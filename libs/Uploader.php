...
class Uploader {
    ...
    private function validate($file = null){
        ...
        if($file == null){
            ...
            if($options['extensions'] && !in_array(strtolower($file['extension']), $options['extensions'])) $errors[] = $this->error_messages['accept_file_types'];
            if($file['type'][0] == "image" && @!is_array(getimagesize($file['tmp']))) $errors[] = $this->error_messages['accept_file_types'];
			/*
            if($options['maxSize'] && $file['size'] > $options['maxSize']) {
				//var_dump($file['size'], $options['maxSize']);
