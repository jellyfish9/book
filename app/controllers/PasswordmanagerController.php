	...
	function postresetlink(){
		...
		if(!empty($_POST['email'])){
			...
			if(!empty($user)){
				...
				$db->update( 'user', array("password_reset_key"=>$password_reset_key) );
				...
				$this->view->page_error = "电子邮件地址未在系统上注册";
				$this->view->render('passwordmanager/index.php',null,"info_layout.php");
			}
		}
		else{
	...
	function updatepassword($password_key=null){
		...
		if(!empty($_POST['password'])){
			...
			if($password == $cpassword){
				...
				$db->update('user',$new_password_data);
				...
				$this->view->page_error = "您的密码确认不一致";
