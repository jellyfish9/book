...
class BookController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$fields = array('id', 	'book_id', 	'name', 	'image', 	'type');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		if(!empty($this->search)){
			$text=$this->search;
			$db->orWhere('id',"%$text%",'LIKE');
			$db->orWhere('book_id',"$text",'=');
			$db->orWhere('name',"%$text%",'LIKE');
			$db->orWhere('type',"$text",'=');
		}
		...
		$records = $db->get('book', $limit, $fields);
		$data = new stdClass;
	...
	function import_data(){
		if(!empty($_FILES['file'])){
			...
			if(!empty($file_path)){
				$db = $this->GetModel();
				if($ext == 'csv'){
					$options = array('table' => 'book', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
					$data = $db->loadCsvData( $file_path , $options , false );
				}
				else{
					$data = $db->loadJsonData( $file_path, 'book' , false );
				}
				if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$rec_id = $db->insert('book',$modeldata);
			if(!empty($rec_id)){
				render_json($rec_id);
			}
			else{
     ...
     * @return View
     */
	function show($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$fields = array('id', 	'book_id', 	'name', 	'image', 	'type');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		if(!empty($this->search)){
